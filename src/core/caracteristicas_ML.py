"""
caracteristicas_ML.py
---------------------
Funciones para generación de features, entrenamiento y predicción con modelos de Machine Learning para trading.
Incluye limpieza de datos, creación de datasets, entrenamiento de Random Forest y predicción de precios.
"""
...
def generar_features(datos_historicos):
    ...
    try:
        ...
        if df_cleaned.isnull().values.any():
            ...
            logging.warning(f"⚠️ Persisten valores NaN en columnas: {nan_columns.tolist()}")
            return None
        ...
        if features.size == 0:
            logging.error("❌ Error: `features` está vacío antes de limpiar.")
            return None
...
def train_random_forest(df):
    """
    Entrena el modelo Random Forest para clasificación.
    """
...
def train_random_forest_regressor(features, prices):
    """
    Entrena modelo Random Forest para regresión con validación cruzada y optimización.
    """
...
# Ejemplo de uso (ajusta la ruta si es necesario)
# df = pd.read_csv("data/historial_trading.csv")  # Asegura que tenga columna 'close'
