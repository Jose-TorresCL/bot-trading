"""
Carga combinada de datos para un único símbolo
---------------------------------------------
Funciones para cargar y combinar datos históricos de trading en formato CSV,
con validación básica de columnas y opción de complementar vía API.
"""
...
def cargar_y_combinar_datos(
    ...
    min_days: int = 30               # intentar asegurar al menos este número de días (si se permite api_fetch)
) -> pd.DataFrame:
    frames = []
    ...
    # a partir de aquí usare siempre la columna 'timestamp'
    ts_col = "timestamp"
    ...
    # Si el rango es menor al mínimo pedido, intentar completar vía API (si está permitido)
    if span_days < int(min_days) and api_fetch and client:
        logger.info("Rango %d días < min_days=%d. Intentando completar vía API...", span_days, min_days)
        try:
            try:
                from src.pipeline.conexion_api import get_historical_data
            except Exception:
            ...
            # calcular un límite aproximado: min_days * 1440 (1m) o el 'limit' proporcionado, con un tope razonable
            approx_limit = max(limit, min(int(min_days) * 1440, 5000))
            df_api2 = get_historical_data(client, symbol=symbol, interval=interval, limit=approx_limit)
            if isinstance(df_api2, list):
                df_api2 = pd.DataFrame(df_api2)
            if df_api2 is not None and not df_api2.empty:
                logger.info("API devolvió %d filas adicionales — concatenando y re-normalizando timestamps.", len(df_api2))
                frames.append(df_api2)
                df = pd.concat(frames, ignore_index=True)
                # volver a parsear timestamp y normalizar
                ts_col2 = _detect_timestamp_col(df)
                if ts_col2 is None:
                    logger.warning("Después de concatenar API no se encontró columna temporal.")
                else:
                    df[ts_col2] = _parse_timestamp(df[ts_col2])
                    if ts_col2 != "timestamp":
                        df.rename(columns={ts_col2: "timestamp"}, inplace=True)
                    df = df.dropna(subset=["timestamp"]).sort_values("timestamp").drop_duplicates(subset=["timestamp"]).reset_index(drop=True)
                    start = df["timestamp"].min(); end = df["timestamp"].max(); span_days = (end - start).days if pd.notna(start) and pd.notna(end) else 0
            else:
                logger.warning("API no devolvió datos adicionales para %s.", symbol)
        except Exception as e:
            logger.warning("Error al intentar completar datos vía API para %s: %s", symbol, e)
    ...
    logger.info(
        "Dataset %s filas tras filtro meses=%s (desde %s hasta %s) — span_days=%s",
        len(df), meses,
        df["timestamp"].min(), df["timestamp"].max(), span_days
    )
