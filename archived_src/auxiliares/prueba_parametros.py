"""
prueba_parametros.py
--------------------
[NO USADO EN EL FLUJO PRINCIPAL]
Script auxiliar para grid search de parámetros de estrategias de trading.
Úsalo solo para optimización manual de parámetros.
"""
...
from src import estrategias_bot1
from src import conexion_api
from src import carga_datos
from src import ciclo_real
import json
import os
import pandas as pd
import traceback
...
def probar_combinacion(params):
    """
    Ejecuta una combinación de parámetros y retorna el resumen de resultados o el error.
    """
    ...
    try:
        ...
        df = backtesting.cargar_y_combinar_datos("historial_trading_limpio.csv", client, meses=12)
...
if __name__ == "__main__":
    ...
    if not os.path.exists("historial_trading_limpio.csv"):
        print("❌ El archivo 'historial_trading_limpio.csv' no existe. Por favor, verifica la ruta.")
        exit()

    df = pd.read_csv("historial_trading_limpio.csv")
    if df.empty:
        print("⚠️ El archivo 'historial_trading_limpio.csv' está vacío. Por favor, verifica los datos.")
        exit()
    ...
    if missing_columns:
        print(f"❌ Faltan las siguientes columnas en 'historial_trading_limpio.csv': {missing_columns}")
        exit()

    print(f"✅ Archivo 'historial_trading_limpio.csv' cargado correctamente con {len(df)} filas.")
    ...
    def mejor_combinacion(resultados, metrica="ganancia_total"):
        ...
        if not mejores:
            return None
        # Penaliza el drawdown: ganancia_total - k * max_drawdown
        k = 1  # Ajusta este valor según tu tolerancia al riesgo
        mejor = max(mejores, key=lambda x: x["ganancia_total"] - k * x["max_drawdown"])
        return mejor
