...
from estrategias_bot import estrategia_compra, estrategia_venta, registrar_decisiones, gestion_riesgo
...
def main():
    ...
    # Inicializa variables para gestión de posición
    mejor_precio = None
    precio_compra = None
    ...
    try:
        ...
        resultado_compra = estrategia_compra(ultima_indicacion)
        es_compra = resultado_compra[0] if isinstance(resultado_compra, tuple) else resultado_compra

        resultado_venta = estrategia_venta(ultima_indicacion)
        es_venta = resultado_venta[0] if isinstance(resultado_venta, tuple) else resultado_venta
        ...
        if es_compra:
            ...
            mejor_precio = precio_actual  # Al abrir la posición
        ...
        # Solo gestiona riesgo si hay una compra previa
        if precio_compra is not None and mejor_precio is not None:
            mejor_precio = max(mejor_precio, precio_actual)  # Actualiza mejor precio
            riesgo = gestion_riesgo(precio_compra, precio_actual, ultima_indicacion, sl_mult=1.5, tp_mult=3, trailing_stop=True, mejor_precio=mejor_precio)
            if riesgo in ["vender_por_perdida", "vender_por_ganancia"]:
                # Ejecuta venta y registra decisión
                registrar_decisiones("venta", precio_actual, ultima_indicacion, riesgo)
                # ...cierra la posición y resetea variables...
                precio_compra = None
                mejor_precio = None
